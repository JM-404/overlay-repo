...
class WebSocketServerManager:
    ...
    def __init__(
        ...
        Args:
            ...
            on_text_callback: Callback(text, client_id) when a text message
    ...
    async def _process_message(
        ...
        try:
            ...
            # keyboard-input mode without running STT.
            if "text" in data and data["text"]:
                if self.on_text_callback:
                    try:
                        await self.on_text_callback(
                            str(data["text"]), client_id
                        )
                    except Exception as e:
                        self.ten_env.log_error(
                            f"Error in text callback: {e}"
                        )
                        await self._send_error(
                            websocket, f"Text processing error: {str(e)}"
                        )
                return
            ...
            if "audio" not in data:
                await self._send_error(
                    websocket,
                    'Missing required field: "audio" (base64) or "text"',
                )
                return
